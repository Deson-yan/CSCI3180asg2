...
public class SurvivalGame {
	...
	private  void init() {
		...
		for (int i = 0; i < (n/2 - 1); i++) {
			teleportObjects[i] = new Human(0, 0, i, this, false);
			teleportObjects[i + n / 2] = new Chark(0, 0, i, this, false);
		}
		teleportObjects[n/2 - 1] = new Human(0, 0, n/2 - 1, this, true);
		teleportObjects[n-1] = new Chark(0, 0, n/2 - 1, this, true);
	...
	private  void gameStart() {
		int turn = 0;
		int numOfAliveHuman = n/2;
		int numOfAliveChark = n/2;
		while (numOfAliveHuman > 0 && numOfAliveChark > 0) {
			...
			numOfAliveChark = 0;
			for (int i = 0; i < n/2; i++) {
				if (((Human) teleportObjects[i]).health > 0)
					numOfAliveHuman += 1;
				if (((Chark) teleportObjects[i+n/2]).health > 0)
					numOfAliveChark += 1;
			}
